...
def main():
    ...
    log.info('elmo loaded')


    def elmoGen(elmo,dataset):
        paragraph_elmos=[]
        question_elmos=[]
        for i,pair in enumerate(dataset):
            paragraph = pair[1]
            question = pair[5]
            paragraph_ids = batch_to_ids([paragraph])
            question_ids = batch_to_ids([question])
            paragraph_elmo = torch.cat(elmo(paragraph_ids)['elmo_representations'],dim=0)
            question_elmo = torch.cat(elmo(question_ids)['elmo_representations'],dim=0)
            paragraph_elmos.append(paragraph_elmo)
            question_elmos.append(question_elmo)
            print('elmo generating: ',i,'/',len(dataset))
        embs = [paragraph_elmos,question_elmos]

        return embs

    train_elmo = elmoGen(elmo,train)
    dev_elmo = elmoGen(elmo,dev)
